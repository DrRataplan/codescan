...
const { evaluateXPathToMap, evaluateXPathToNodes, evaluateXPathToString, parseScript } = fontoxpath;

type FunctionCallGraph = Map<string, {
	incoming: Set<string>,
	outgoing: Set<string>,
	isRoot: boolean
}>;

function addEdge(graph: FunctionCallGraph, callingFunction: string, calledFunction: string, callingFunctionIsRoot: boolean) {
	if (!graph.get(callingFunction)) {
		graph.set(callingFunction, { incoming: new Set(), outgoing: new Set(), isRoot: callingFunctionIsRoot })
	}
	const entryForCaller = graph.get(callingFunction)!;
	entryForCaller.outgoing.add(calledFunction);

	if (!graph.get(calledFunction)) {
		graph.set(calledFunction, { incoming: new Set(), outgoing: new Set(), isRoot: false })
	}
	const entryForCallee = graph.get(calledFunction)!;
	entryForCallee.incoming.add(callingFunction);
}

export async function scanXQueryFile(fileName: string, contents: string, functionCallGraph: FunctionCallGraph) {
	let script: Element;
	try {
		script = parseScript(contents, { annotateAst: true, namespaceResolver }, document);
	} catch (err: unknown) {
...
) => map:merge()`, script, null, null, { namespaceResolver });

	// First: declared functions
	for (const functionDeclaration of evaluateXPathToNodes('descendant-or-self::functionDecl', script, null, null, { namespaceResolver })) {
		const callingFunctionName = evaluateXPathToString('"Q{" || $namespaceByPrefix(functionName/@xqx:prefix/string()) || "}" || functionName', functionDeclaration, null, { namespaceByPrefix }, { namespaceResolver });
		for (const call of evaluateXPathToNodes(
			...
			const calledFunctionName = evaluateXPathToString('"Q{" || functionName/@xqx:URI || "}" || functionName', call, null, null, { namespaceResolver });
			addEdge(functionCallGraph, callingFunctionName, calledFunctionName, false);
		}
	}
